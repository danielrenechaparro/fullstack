// Comenzar una app con angular MVC

/*Creacion de un modulo de angular, como var global, palabra reservada angular, 
nombre_modulo, lista dependencias
*/
var app = angular.module("nombre_modulo", []);

function recorrer_json(json){
    var arreglo_resultante = [];
    for(let i=0; i<json.length; i++){
        //arreglo_resultante.push("http://www.countryflags.io/"+json[i]["home_team"]["code"]+"/flat/64.png"); // Se llama la clave deseada del json
        
        // arreglo_resultante.push(json[i]["home_team"]["code"]); // Se llama la clave deseada del json
        // arreglo_resultante.push(json[i].away_team.code); // Se puede llamar como propiedad, ya que en js json es un objeto, entonces se usa la propiedad
        arreglo_resultante.push(
            {
                "nombre":json[i]["home_team_country"],
                "cod":json[i]["home_team"]["code"],
            }
        ); 
        arreglo_resultante.push(
            {
                "nombre":json[i]["away_team_country"],
                "cod":json[i].away_team.code,
            }
        );
    }
...
/*
Creo el controlador del modulo, le pongo nombre y le creo una funcion con la varible global
*/
app.controller("nombreCtrl", function($scope){
    $scope.nombre_var = json;
    $scope.arreglo = recorrer_json(json);;
})